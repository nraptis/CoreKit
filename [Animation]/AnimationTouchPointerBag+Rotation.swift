...
extension AnimationTouchPointerBag {
    ...
    func captureStart_PrepareRotate(averageX: Float,
                                    averageY: Float) {
    ...
    // [Animation Mode Verify] 12-19-2024
    // Looks good, no problem. I've read each line.
    func captureStart_Rotate(animationWad: AnimationWad, rotation: Float) {
        ...
        let grabDragPowerPercentLinear = AnimationWad.getGrabDragPowerPercentLinear(userGrabDragPower: animationWad.grabDragPower)
        
        // We're converting to the range [0.15...1.0]
        // This means that at 0.0, there is still some small effect...
        let grabDragRotateFactor = AnimationWad.getGrabDragRotateFactor(grabDragPowerPercentLinear: grabDragPowerPercentLinear)
        
        // We're adjusting the positive rotation based on
        // the power which the user has selected.
        let baseRotationU1 = AnimationWad.animationCursorFalloffRotation_U1 * grabDragRotateFactor
        let baseRotationU2 = AnimationWad.animationCursorFalloffRotation_U2 * grabDragRotateFactor
        let baseRotationU3 = AnimationWad.animationCursorFalloffRotation_U3 * grabDragRotateFactor
        
        // We're adjusting the negative rotation based on
        // the power which the user has selected.
        let baseRotationD1 = AnimationWad.animationCursorFalloffRotation_D1 * grabDragRotateFactor
        let baseRotationD2 = AnimationWad.animationCursorFalloffRotation_D2 * grabDragRotateFactor
        let baseRotationD3 = AnimationWad.animationCursorFalloffRotation_D3 * grabDragRotateFactor
    ...
    // [Animation Mode Verify] 12-19-2024
    // Looks good, no problem. I've read each line.
    func captureTrack_PrepareRotate(animationWad: AnimationWad, averageX: Float, averageY: Float) -> Bool {
    ...
    @MainActor func captureTrack_Rotate(animationWad: AnimationWad,
        ...
        // We're converting to the range [0.1...1.0]
        // This means that at 0.0, there is still some small effect...
        let grabDragRotateFactor = AnimationWad.getGrabDragRotateFactor(grabDragPowerPercentLinear: grabDragPowerPercentLinear)
        ...
        if (captureStartJiggleAnimationCursorRotation > baseRotationU1) && (newRotation < captureStartCursorFalloffRotation_U1) {
            captureStart_PrepareRotate(averageX: averageX,
                                       averageY: averageY)
            captureStart_Rotate(animationWad: animationWad, rotation: newRotation)
        }
        ...
        animationWad.animationCursorRotation = newRotation
    ...
    // [Animation Mode Verify] 12-19-2024
    // Looks good, no problem. I've read each line.
    @MainActor func registerContinuousStartRotation(animationWad: AnimationWad,
                                                    jiggleDocument: AnimationControllerJiggleDocument) {
        ...
        let fixedRotation = fixRotation_NearZero(rotation: animationWad.animationCursorRotation)
