...
extension AnimationTouchPointerBag {
    ...
    func captureStart_PrepareScale(averageX: Float,
                                   averageY: Float) {
    ...
    // [Animation Mode Verify] 12-19-2024
    // Looks good, no problem. I've read each line.
    func captureStart_Scale(animationWad: AnimationWad, scale: Float) {
        ...
        // We're converting to the range [0.10...1.0]
        // This means that at 0.0, there is still some small effect...
        let grabDragScaleFactor = AnimationWad.getGrabDragScaleFactor(grabDragPowerPercentLinear: grabDragPowerPercentLinear)
        ...
        let baseScaleStepU1 = AnimationWad.animationCursorFalloffScale_U1 - 1.0
        let baseScaleU1 = 1.0 + baseScaleStepU1 * grabDragScaleFactor
        let baseScaleStepU2 = AnimationWad.animationCursorFalloffScale_U2 - 1.0
        let baseScaleU2 = 1.0 + baseScaleStepU2 * grabDragScaleFactor
        let baseScaleStepU3 = AnimationWad.animationCursorFalloffScale_U3 - 1.0
        ...
        // the resting scale, so we adjust based on
        // how far away the scale is from 1.0...
        let baseScaleStepD1 = 1.0 - AnimationWad.animationCursorFalloffScale_D1
        let baseScaleD1 = 1.0 - baseScaleStepD1 * grabDragScaleFactor
        let baseScaleStepD2 = 1.0 - AnimationWad.animationCursorFalloffScale_D2
        let baseScaleD2 = 1.0 - baseScaleStepD2 * grabDragScaleFactor
        let baseScaleStepD3 = 1.0 - AnimationWad.animationCursorFalloffScale_D3
    ...
    // [Animation Mode Verify] 12-19-2024
    // Looks good, no problem. I've read each line.
    func captureTrack_PrepareScale(animationWad: AnimationWad, averageX: Float, averageY: Float) -> AnimationTouchPointerScaleResponse {
        ...
        let scaleWeightUnit = AnimationWad.getAnimationCursorScaleWeightUnit(measuredSize: animationWad.measuredSize)
    ...
    @MainActor func captureTrack_Scale(animationWad: AnimationWad,
        ...
        animationWad.animationCursorScale = newScale
    ...
    // [Animation Mode Verify] 12-19-2024
    // Looks good, no problem. I've read each line.
    @MainActor func registerContinuousStartScale(animationWad: AnimationWad,
                                                 jiggleDocument: AnimationControllerJiggleDocument) {
        
        // We're converting the scale value to
        // user-slider values. When the slider is
        // at 50% (the middle), that's "resting" / 1.0
        let scale = animationWad.animationCursorScale
