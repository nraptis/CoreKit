//
//  Jiggle+PointNearestInsertIndex.swift
//  Yo Mamma Be Ugly
//
...
public extension Jiggle {
    ...
    func calculateNearestInsertIndex(_ x: Float, _ y: Float) -> Int? {
        ...
        var area = Float(0.0)
        for index1 in 0..<jigglePointCount {
            var index2 = index1 + 1
            if index2 == jigglePointCount {
                index2 = 0
            }
            let jigglePoint1 = jigglePoints[index1]
            let jigglePoint2 = jigglePoints[index2]
            var point1 = jigglePoint1.point
            point1 = transformPoint(point: point1)
            var point2 = jigglePoint2.point
            point2 = transformPoint(point: point2)
        ...
        if tempIntegerCount > 3 {
            ...
            if lineSegmentLeft.isIllegal {
                ...
                let isEnd: Bool
                if isClockwise {
                    if tempIntegers[0] == 0 && (tempIntegers[1] == jigglePointCount - 1) {
