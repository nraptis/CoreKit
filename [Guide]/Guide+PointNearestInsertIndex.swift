...
public extension Guide {
    ...
    func calculateNearestInsertIndex(_ x: Float, _ y: Float) -> Int? {
        ...
        var area = Float(0.0)
        for index1 in 0..<guidePointCount {
            var index2 = index1 + 1
            if index2 == guidePointCount {
                index2 = 0
            }
            let jigglePoint1 = guidePoints[index1]
            let jigglePoint2 = guidePoints[index2]
            let point1 = jigglePoint1.point
            let point2 = jigglePoint2.point
        ...
        if tempIntegerCount > 3 {
            ...
            if lineSegmentLeft.isIllegal {
                ...
                let isEnd: Bool
                if isClockwise {
                    if tempIntegers[0] == 0 && (tempIntegers[1] == guidePointCount - 1) {
