...
public class JiggleRenderer {
    ...
    public func pre_prepare(renderInfo: JiggleRenderInfo,
        ...
        if renderInfo.isShowingJigglePoints {
            ...
            for jigglePointIndex in 0..<jigglePointCount {
                ...
                var renderPoint = jigglePoint.point
                ...
                case .unselected:
                    if isJiggleSelected == true {
                        if selectedJigglePointIndex == jigglePointIndex {
                ...
                jigglePoint.renderX = renderPoint.x
                jigglePoint.renderY = renderPoint.y
                jigglePoint.renderPointSelected = isPointSelected
                ...
                jigglePoint.renderTanInSelected = isTanInSelected
                jigglePoint.renderTanOutSelected = isTanOutSelected
        ...
        if renderInfo.isShowingJigglePointTanHandles {
            for jigglePointIndex in 0..<jigglePointCount {
                ...
                var tanHandleIn = jigglePoint.getTanHandleIn()
                tanHandleIn = Math.transformPoint(point: tanHandleIn,
                                                  translation: jiggleCenter,
                                                  scale: jiggleScale,
                                                  rotation: jiggleRotation)
                
                jigglePoint.renderTanInX = tanHandleIn.x
                jigglePoint.renderTanInY = tanHandleIn.y
                
                var tanHandleOut = jigglePoint.getTanHandleOut()
                tanHandleOut = Math.transformPoint(point: tanHandleOut,
                                                   translation: jiggleCenter,
                                                   scale: jiggleScale,
                                                   rotation: jiggleRotation)
                
                jigglePoint.renderTanOutX = tanHandleOut.x
                jigglePoint.renderTanOutY = tanHandleOut.y
                
                var tanNormalsIn = jigglePoint.getTanHandleNormalsIn()
                tanNormalsIn = Math.transformVector(vector: tanNormalsIn,
                                                    scale: 1.0,
                                                    rotation: jiggleRotation)
                
                
                jigglePoint.renderTanNormalInX = tanNormalsIn.x
                jigglePoint.renderTanNormalInY = tanNormalsIn.y
                
                var tanNormalsOut = jigglePoint.getTanHandleNormalsOut()
                tanNormalsOut = Math.transformVector(vector: tanNormalsOut,
                                                     scale: 1.0,
                                                     rotation: jiggleRotation)
                
                jigglePoint.renderTanNormalOutX = tanNormalsOut.x
                jigglePoint.renderTanNormalOutY = tanNormalsOut.y
            }
        }
    }
    ...
    public func prepare(renderInfo: JiggleRenderInfo,
        ...
        preparePoints(renderInfo: renderInfo,
                      jigglePoints: jigglePoints,
                      jigglePointCount: jigglePointCount,
